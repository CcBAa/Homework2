    ...
    Scaffold(
        ...
        NavHost(navController = navController, startDestination = MainScreen.Start.name, modifier = Modifier.padding(innerPadding)) {
            composable(route = MainScreen.Start.name) {
                StartItem(names = spots) { index ->
                    navController.navigate("${MainScreen.Detail.name}/$index")
                }
            }
            composable(
                route = "${MainScreen.Detail.name}/{index}",
                arguments = listOf(navArgument("index") { type = NavType.IntType })
            ) { backStackEntry ->
                val index = backStackEntry.arguments?.getInt("index") ?: 0
                val name = spots.getOrNull(index) ?: ""
                DetailScreen(index = index, name = name, navigateUp = { navController.navigateUp() })
            }
        }
    }
}
